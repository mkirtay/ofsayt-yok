...
/** Round metninin normalize edilmiÅŸ turu; bilinmeyense null. `R32 / 1/16` yok sayÄ±lÄ±r. */
...
export type BracketPair = {
  key: string;
  /** AynÄ± eÅŸleÅŸmenin iki ayaÄŸÄ± birleÅŸmiÅŸ olabilir; iki maÃ§ da tutulur */
  matches: Match[];
  home?: { id?: number; name?: string; logo?: string };
  away?: { id?: number; name?: string; logo?: string };
  /** Aggregate skor (iki maÃ§ toplamÄ±), yoksa son maÃ§ skoru */
  scoreText?: string;
...
/** Ä°ki takÄ±m arasÄ±ndaki stabil Ã§ift anahtarÄ± (sÄ±ralÄ± Ã§ift id) */
...
function buildPair(matches: Match[]): BracketPair {
  ...
  if (sorted.some((m) => m.status === 'FINISHED') && agg) {
    ...
    if (agg.h > agg.a) winner = 'home';
      ...
      // EÅŸitse tekil maÃ§Ä±n outcomes.full_time / et / pens kullan
...
/** Ã–nceki turun tamamlanmÄ±ÅŸ Ã§iftlerinden sÄ±radaki tur iÃ§in sentetik eÅŸleÅŸmeler Ã¼retir. */
...
/**
 * Knockout maÃ§larÄ±nÄ± turlara bÃ¶ler ve iki ayaÄŸÄ± olan eÅŸleÅŸmeleri birleÅŸtirir.
 * `R16 â†’ QF â†’ SF â†’ F` turlarÄ± her zaman dÃ¶ndÃ¼rÃ¼lÃ¼r; maÃ§ yoksa Ã¶nceki turun
 * kazananlarÄ±ndan sentetik eÅŸleÅŸmeler tÃ¼retilir.
 */
...
/** MaÃ§ listesinin ana sayfadaki gibi sÄ±ralanmasÄ±: canlÄ± â†’ planlanmÄ±ÅŸ â†’ bitmiÅŸ (en yeni Ã¼stte) */
export function sortMatchesForUefaList(matches: Match[]): Match[] {
  ...
  return [...matches].sort((a, b) => {
    ...
    if (ra === 3) {
      // BitmiÅŸ maÃ§lar: en yeni Ã¼stte
...
/**
 * Lig seÃ§imine gÃ¶re canlÄ± + fikstÃ¼r + geÃ§miÅŸ birleÅŸimi.
 * `fixture_id` veya `id` Ã¼zerinden tekilleÅŸtirir; canlÄ±/bitmiÅŸ satÄ±r fikstÃ¼rÃ¼ ezer.
 */
