...
/** Yol + include'da aranan parça → TTL. İlk eşleşen kural geçerli. */
const CACHE_RULES: { path: RegExp; include: string; ttl: number }[] = [
...
export function statsCacheTtl(path: string, query: Record<string, string | string[] | undefined>): number | null {
  ...
  const p = norm(path);
  return CACHE_RULES.find((r) => r.path.test(p) && include.includes(r.include))?.ttl ?? null;
}
